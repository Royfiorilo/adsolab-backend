...
class LinearizationTest(unittest.TestCase):
    ...
    def generate_freundlich_data(self, kf=0.5, nf=2, num_points=10, noise_level=0.05):
    ...
    def test_linearization_HaneseWoolf(self):
        ...
        params = ('HaneseWoolf Linearization', 'ce/qe = (1/qmax) * ce + 1 / (qmax * k)', 'Linealizacion del modelo de Langmuir', {"x": "ce", "y": "ce/qe", "m": "1/qmax","b": "1/(qmax * k)"}, 1)
        ...
        self.assertAlmostEqual(result['qmax'], q_max, places=10)
        self.assertAlmostEqual(result['k'], k, places=4)
    ...
    def test_linearization_Lineweaver_Burk(self):
        ...
        params = ('Lineweaver-Burk linearization', '1 / qe = (1 / k * qmax) * (1 / ce) + 1 / qmax', 'Linealizacion del modelo de Langmuir', {"x": "1/ce", "y": "1/qe", "m": "1/(k*qmax)", "b": "1/qmax"}, 1)
    ...
    def test_linearization_freundlich(self):
        ...
        params = ('Freundlich linearization', 'log(qe) = log(kf) + 1/nf * log(ce)', 'Linealizacion del modelo de Freundlich', {"x": "log(ce)", "y": "log(qe)", "m": "1 / nf", "b": "log(kf)"}, 2)
        ...
        sample = SampleEntity(ce, qe)
        result = linearization.run(sample)
        self.assertAlmostEqual(result['nf'], nf, places=10)
        self.assertAlmostEqual(result['kf'], kf, places=4)
